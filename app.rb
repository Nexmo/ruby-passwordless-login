...
# enable sessions and set the session secret
enable :sessions
...
# Index
# - tries to show the current user's
#   phone number if it is present
#
get '/' do
...
# Login
# - presents the user with a form to
...
# Start Verification
# - starts the verification process, sending
#   a code to the user's phone number and
#   then redirecting them to verify the code
#
post '/start_verification' do
  ...
  if response['status'] == '0'
    # save the number so we can show it
    # once the user is logged in
    session[:number] = params['number']
    # save the verification ID so we can verify the user's
    # code against it in the next step
...
# Code Verify
# - shows the user a form to fill in the code
#   they received via text message
#
get '/verify' do
  erb :verify
end
...
#   confirming the user submitted code
...
# Logout
# - clears the session and redirects the
